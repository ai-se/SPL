 ...
 */
public class SIP_IBEA extends Algorithm {
     ...
     */
    public SIP_IBEA(Problem problem) {
        super (problem) ;
    } // Spea2
    ...
    public void removeWorst(SolutionSet solutionSet) {
        ...
        List<Double> conVios = new ArrayList<>(solutionSet.size());
        for (int i = 0; i < solutionSet.size(); i++){
            conVios.add(solutionSet.get(i).getObjective(0));
        }

        double d = Collections.max(conVios);
        List<Integer> worseIndices = new ArrayList<>(solutionSet.size());
        for (int i = 0; i < solutionSet.size(); i++){
            if (Math.abs(solutionSet.get(i).getObjective(0) - d) < 0.01){
                worseIndices.add(i);
            }
        }

        for (int i: worseIndices) {
