 ...
 */
public class PEN_IBEA extends Algorithm {
     ...
     */
    public PEN_IBEA(Problem problem) {
        super (problem) ;
    } // Spea2
    ...
    public SolutionSet execute() throws JMException, ClassNotFoundException{
        ...
        while (evaluations < maxEvaluations){
            ...
            while (offSpringSolutionSet.size() < populationSize){
                ...
                } while (j < jmetal.metaheuristics.ibea.IBEA.TOURNAMENTS_ROUNDS); // do-while
                int k = 0;
                do{
                    k++;
                    parents[1] = (Solution)selectionOperator.execute(archive);
                } while (k < jmetal.metaheuristics.ibea.IBEA.TOURNAMENTS_ROUNDS); // do-while
            ...
            Binary[] b = new Binary[solutionSet.size()];
            for (int i = 0; i < solutionSet.size();i++){
                Variable v = solutionSet.get(i).getDecisionVariables()[0];
                b[i] = (Binary) v;
            }
            ((ProductLineProblem)problem_).updateConstraintWeight(b);
        } // while

        for (Iterator<Solution> it = archive.iterator(); it.hasNext();){
            Solution sltn = it.next();
            Variable[] vars = sltn.getDecisionVariables();
            Binary bin = (Binary) vars[0];
            sltn.setObjective(0, ((ProductLineProblem)problem_).numViolatedConstraints(bin));
        }
