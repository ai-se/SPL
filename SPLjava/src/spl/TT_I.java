 ...
 */
public class TT_I extends Algorithm{
    ...
    public SolutionSet execute() throws JMException, ClassNotFoundException{
        ...
        while (evaluations < maxEvaluations){
            ...
            Random r = new Random();
            ArrayList<Integer>[] map = new ArrayList[archive.size()];
            ...
            while (offSpringSolutionSet.size() < populationSize){
                ...
                for (int i = 0; i < map.length; i++){
                    ...
                    int tmp = 0;
                    for (Integer xx: map[i]){
                        if (map[r_0].contains((Object)xx)) {
                            tmp += 1;
                        }
                    }
                    if (tmp < min){
                ...
                parents[1] = archive.get(cursor);
                // find the one whose violated constraint differ form parents[0]
...
//                evaluations += 2;
                if ((new DominanceComparator()).compare(parents[0], parents[1]) == -1){
