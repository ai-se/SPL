...
def get_references(objNum):
    ...
    m = objNum
    divs = DIVISIONS[m]
    refs = cover(m, divs[0], divs[1])
    return map(ReferencePoint, refs)


def generate_reference_points(num_objs, num_divisions_per_obj=4, newref=False):
    if newref:
